...
func _physics_process(delta: float) -> void:
	ball_movement.rpc()
	foot_ball.move_and_collide(foot_ball.linear_velocity*delta, false, 0.001, true)
	#if collision is Bar:
		#if collision.get_multiplayer_authority() == Game.get_current_player().id:
			#Debug.log("collision")
			#last_player_id = Game.get_current_player().id
			#pass
	pass
